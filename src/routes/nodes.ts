...
async function getGatherableItems(
  ...
  const newGatherableItems: NewGatherableItemBaseDocument[] = gatherableItemsArray.map(
    (item: any) => {
...
async function getNodesWithItemData(...servers: string[]){
  const [gatherableItems, nodes] = await Promise.all([getGatherableItems(...servers), NodeHelpers.getAllNodes()]);
  return nodes.map(
      ({
        filters,
        location,
